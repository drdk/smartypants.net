...
namespace SmartyPantsNet
{
    public class SmartyPants
    {
        ...
        protected string EducateQuotes(string text)
        {
            ...
            text = new Regex("(\\s|--|\u2013|\u2014)'(?=\\w)").Replace(text, "$1" + sqOpen);

            // Single closing quotes:
            text = new Regex("(" + closeClass + ")?'(?(1)|(?=\\s|s\\b))", RegexOptions.IgnoreCase).Replace(text, "$1" + sqClose);

            // Any remaining single quotes should be opening ones:
            text = text.Replace("'", sqOpen);

            // Get most opening double quotes:
            text = new Regex("(\\s|--|\u2013|\u2014)\"(?=\\w)").Replace(text, "$1" + dqOpen);

            // Double closing quotes:
            text = new Regex("(" + closeClass + ")?\"(?(1)|(?=\\s))").Replace(text, "$1" + dqClose);
